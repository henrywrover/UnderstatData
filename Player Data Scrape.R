...
match_list <- as.list(c((max(old_data$match_id)+1):18000))
...
df <- stats %>%
  ...
  select(player, goals, xG, shots, assists, xA, key_passes, time, position, h_a, yellow_card, red_card, key_passes,
         match_id, league, date_of_match) %>%
  rbind(old_data) %>%
  arrange(-match_id)
